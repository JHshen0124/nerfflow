...
class NerfJobs {
public:
	NerfJobs(ETestbedMode mode = ETestbedMode::None);
	~NerfJobs();

	NerfJobs(ETestbedMode mode, const fs::path& data_path) : NerfJobs(mode) { load_training_data(data_path); }
	NerfJobs(ETestbedMode mode, const fs::path& data_path, const fs::path& network_config_path) : NerfJobs(mode, data_path) { reload_network_from_file(network_config_path); }
	NerfJobs(ETestbedMode mode, const fs::path& data_path, const nlohmann::json& network_config) : NerfJobs(mode, data_path) { reload_network_from_json(network_config); }
	...
	public:
		...
		int j1(NerfNetwork<precision_t>& network,
			const BoundingBox& render_aabb,
			const mat3& render_aabb_to_local,
		...
		template <typename T>
		int j20(cudaStream_t stream, 
				const tcnn::GPUMatrixDynamic<float>& input, 
				tcnn::GPUMatrixDynamic<T>& output,
				NerfNetwork<precision_t>& network);
		template <typename T>
		int j21(cudaStream_t stream, 
				const tcnn::GPUMatrixDynamic<float>& input, 
				tcnn::GPUMatrixDynamic<T>& rgb_network_input,
				NerfNetwork<precision_t>& network);
		template <typename T>
		int j30(cudaStream_t stream, 
				const tcnn::GPUMatrixDynamic<T>& input, 
				tcnn::GPUMatrixDynamic<T>& output,
				NerfNetwork<precision_t>& network,uint32_t batch_size);
		template <typename T>
		int j31(cudaStream_t stream, const tcnn::GPUMatrixDynamic<T>& rgb_network_input, tcnn::GPUMatrixDynamic<T>& rgb_network_output,
		...
		std::pair<vec2,vec2> j00(const ivec2& resolution,
	...
	int render_to_test(int width, int height, int spp, bool linear, float start_t, float end_t, float fps, float shutter_fraction);
