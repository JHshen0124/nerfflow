...
NerfJobs::NetworkDims NerfJobs::network_dims_image() const {
	NetworkDims dims;
	dims.n_input = 2;
	dims.n_output = 3;
	dims.n_pos = 2;
...
__global__ void halton23_kernel_jobs(uint32_t n_elements, size_t base_idx, vec2* __restrict__ output) {
...
__global__ void sobol2_kernel_jobs(uint32_t n_elements, size_t base_idx, uint32_t seed, vec2* __restrict__ output) {
	uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i >= n_elements) return;
...
__global__ void zip_kernel_jobs(uint32_t n_elements, const float* __restrict__ in, vec2* __restrict__ output) {
	uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i >= n_elements) return;
...
__global__ void stratify2_kernel_jobs(uint32_t n_elements, uint32_t log2_batch_size, vec2* __restrict__ inout) {
	uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i >= n_elements) return;
...
__global__ void init_image_coords_jobs(
...
__global__ void shade_kernel_jobs_image(ivec2 resolution, const vec2* __restrict__ positions, const vec3* __restrict__ colors, vec4* __restrict__ frame_buffer, bool linear_colors) {
	uint32_t x = threadIdx.x + blockDim.x * blockIdx.x;
	uint32_t y = threadIdx.y + blockDim.y * blockIdx.y;
...
__global__ void eval_image_kernel_jobs_and_snap(uint32_t n_elements, const T* __restrict__ texture, vec2* __restrict__ positions, ivec2 resolution, float* __restrict__ result, bool snap_to_pixel_centers, bool linear_colors) {
	uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i >= n_elements) return;
...
void NerfJobs::train_image(size_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
	...
	auto generate_training_data = [&]() {
		if (m_image.random_mode == ERandomMode::Halton) {
			linear_kernel(halton23_kernel_jobs, 0, stream, n_elements, (size_t)batch_size * m_training_step, m_image.training.positions.data());
		} else if (m_image.random_mode == ERandomMode::Sobol) {
			linear_kernel(sobol2_kernel_jobs, 0, stream, n_elements, (size_t)batch_size * m_training_step, m_seed, m_image.training.positions.data());
		} else {
			...
			if (m_image.random_mode == ERandomMode::Stratified) {
				...
				if (!is_pot(batch_size, &log2_batch_size)) {
					...
					linear_kernel(stratify2_kernel_jobs, 0, stream, n_elements, log2_batch_size, m_image.training.positions.data());
				}
			}
		}

		if (m_image.type == EDataType::Float) {
			linear_kernel(eval_image_kernel_jobs_and_snap<float, 3>, 0, stream,
				n_elements,
			...
			linear_kernel(eval_image_kernel_jobs_and_snap<__half, 3>, 0, stream,
				n_elements,
...
void NerfJobs::render_image(
	...
	init_image_coords_jobs<<<blocks, threads, 0, stream>>>(
		render_buffer.spp,
	...
	shade_kernel_jobs_image<<<blocks, threads, 0, stream>>>(
		res,
		m_image.render_coords.data(),
...
void NerfJobs::load_image(const fs::path& data_path) {
...
void NerfJobs::load_exr_image(const fs::path& data_path) {
	if (!data_path.exists()) {
...
void NerfJobs::load_stbi_image(const fs::path& data_path) {
	if (!data_path.exists()) {
		throw std::runtime_error{fmt::format("Image file '{}' does not exist.", data_path.str())};
	}
...
void NerfJobs::load_binary_image(const fs::path& data_path) {
	if (!data_path.exists()) {
		throw std::runtime_error{fmt::format("Image file '{}' does not exist.", data_path.str())};
	}
...
__global__ void image_coords_from_idx_jobs(const uint32_t n_elements, uint32_t offset, vec2* __restrict__ pos, ivec2 resolution) {
	uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i >= n_elements) return;
...
__global__ void image_mse_kernel_jobs(const uint32_t n_elements, const vec3* __restrict__ target, const vec3* __restrict__ prediction, float* __restrict__ result, bool quantize_to_byte) {
	uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i >= n_elements) return;
...
float NerfJobs::compute_image_mse(bool quantize_to_byte) {
	const uint32_t n_output_dims = 3;
	const uint32_t n_input_dims = 2;
	...
	for (uint32_t i = 0; i < n_batches; ++i) {
		...
		linear_kernel(image_coords_from_idx_jobs, 0, nullptr,
			batch_size,
			offset,
			pos.data(),
			m_image.resolution
		...
		if (m_image.type == EDataType::Float) {
			linear_kernel(eval_image_kernel_jobs_and_snap<float, 3>, 0, nullptr,
				batch_size,
				(float*)m_image.data.data(),
				pos.data(),
				m_image.resolution,
			...
			linear_kernel(eval_image_kernel_jobs_and_snap<__half, 3>, 0, nullptr,
				batch_size,
				(__half*)m_image.data.data(),
				pos.data(),
				m_image.resolution,
		...
		linear_kernel(image_mse_kernel_jobs, 0, nullptr,
			batch_size,
			targets.data(),
			predictions.data(),
			se.data() + offset,
